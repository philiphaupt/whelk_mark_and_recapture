...
generate_circular_plot <- function(n_points, color_ratio, max_lim, min_lim, sample_section, pot_number) {
  ...
  ggplot(data, aes(x = X, y = Y)) +
    ...
    labs(x = "", y = "", title = paste0("Whelks in ", sample_section,", Pot number: ", pot_number))
}
...
for (i in 1:num_sampling_sections) {
  sample <- paste0("Sample Section ", i)
  ...
  for (j in 1:num_pots) {
    ...
    print(generate_circular_plot(n_points, color_ratio, max_lim, min_lim, sample_section = sample, pot_number = j))
